...
5. Create the image
       ...
       poudriere image -t firmware -j rpi -s 2g -p latest -h raspberrypi -n raspberrypi \
           -f pkglist -c overlaydir -B pre-script-ufs.sh

   If ZFS is desired

       poudriere image -t zfs -j rpi -s 2g -p latest -h raspberrypi -n raspberrypi \
...
5. Create a boot environment (BE)
       ...
       poudriere image -t zfs+send+be -j rpi -s 2g -p latest -h raspberrypi -n raspberrypi \
           -f pkglist -c overlaydir -B pre-script-zfs.sh
